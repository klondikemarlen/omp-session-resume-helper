...
test("restore previews and launches each valid saved session in a Ptyxis window", async () => {
  ...
  registerSessionCommands(pi, {
    ...
    ompPath: "/home/linuxbrew/.linuxbrew/bin:/usr/bin",
  })
  ...
  assert.deepEqual(context.confirmations, [{
    message: "Ptyxis will open one new window per session:\n- /worktree/one: omp --resume 'first' (using current OMP session PATH)\n- /worktree/Marlen's project: omp --resume 'second' (using current OMP session PATH)",
    title: "Restore saved OMP sessions in Ptyxis?",
  }])
  assert.deepEqual(pi.execCalls, [
    {
      args: ["--new-window", "--working-directory", "/worktree/one", "--", "/usr/bin/env", "PATH=/home/linuxbrew/.linuxbrew/bin:/usr/bin", "omp", "--resume", "first"],
      command: "ptyxis",
    },
    {
      args: ["--new-window", "--working-directory", "/worktree/Marlen's project", "--", "/usr/bin/env", "PATH=/home/linuxbrew/.linuxbrew/bin:/usr/bin", "omp", "--resume", "second"],
      command: "ptyxis",
    },
  ])
  assert.deepEqual(notifications, [{
...
test("resume command parsing preserves shell-quoted values", () => {
  ...
  assert.equal(
    ...
    "Ptyxis will open one new window per session:\n- /worktree/project: omp --resume 'session' (using current OMP session PATH)",
  )
})
